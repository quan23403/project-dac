      ...
      <v-tab-item v-for="(table, index) in tables" :key="index">
        ...
        >
          ...
          <template v-slot:[`item.actions`]="{ item }">
            ...
            <v-icon size="small" @click="deleteItem(item.campaignId)">
              mdi-delete
            </v-icon>
          </template>
        </v-data-table>
...
const tables = ref([
  {
    ...
    apiUrl: "http://localhost:8080/category-binding/campaign-category",
    headers: [
...
const fetchCategoryCampaign = async () => {
  try {
    const response = await getAllCategoryCampaign();
